        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <Card className="w-full max-w-lg">
                ...
                <div className="mt-4 text-lg font-semibold">
                    {(userType === 'BUYER' && contract.buyerName === contract.buyerName) || (userType !== 'BUYER' && contract.buyerName !== contract.buyerName) ? (
                        <>Estimated Payment: PHP {(quantity * contract.pricePerKg).toLocaleString()}</>
                    ) : (
                        <>Estimated Payout: PHP {(quantity * contract.pricePerKg).toLocaleString()}</>
                    )}
                </div>
...
export const FuturesMarket: React.FC<{ user: User }> = ({ user }) => {
  ...
  const isBuyerContract = (buyerName: string) => {
    // Hardcoded buyer names from mock data
    return buyerName === 'Global Fish Inc.' || buyerName === 'Seafood Exporters';
  };

  const isSellContractForFisherman = (contractBuyerName: string, currentUser: User) => {
    return currentUser.type === 'FISHERMAN' && contractBuyerName === currentUser.name;
  };
  ...
  const buyerContracts = contracts.filter(c => isBuyerContract(c.buyerName));
  const fishermanContracts = contracts.filter(c => !isBuyerContract(c.buyerName));
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                <Card key={contract.id} className="flex flex-col justify-between">
                    ...
                    <button 
                        ...
                        className={`mt-6 w-full py-2 px-4 font-semibold rounded-lg transition border ${
                            contract.buyerName === user.name || user.type === 'FISHERMAN'
